...
public class grass implements Tile {
    ...
    public BufferedImage getTileImage(int xPos, int yPos) {
        ...
        try {
            if (!Level.getSmallTile(xPos, yPos - 1).getNameID().equals(NAMEID)) {
                offset += 1;
            }
        } catch (Exception e) {
            offset += 1;
        ...
        try {
            if (!Level.getSmallTile(xPos + 1, yPos).getNameID().equals(NAMEID)) {
                offset += 2;
            }
        } catch (Exception e) {
            offset += 2;
        ...
        if (offset == 0) {
            offset = 16;
            try {
                if (!Level.getSmallTile(xPos - 1, yPos - 1).getNameID().equals(NAMEID)) {
                    offset += 1;
                }
            } catch (Exception e) {
                offset += 1;
